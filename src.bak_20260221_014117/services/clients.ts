...
export class ClientsService {
  async create(userId: string, data: ClientCreateDTO): Promise<Client> {
    const pool = getPool();
    const id = uuidv4();
    await pool.execute(
    ...
    return this.getById(id, userId) as Promise<Client>;
  }

  async getById(id: string, userId: string): Promise<Client | null> {
    const pool = getPool();
    const [rows] = await pool.query<RowDataPacket[]>(
      "SELECT * FROM clients WHERE id = ? AND user_id = ? AND is_active = TRUE", [id, userId]
    );
  ...
  async getAll(userId: string, filters: ClientFilters = {}): Promise<{ clients: Client[]; total: number }> {
    const pool = getPool();
    const { status, source, company_id, search, page = 1, limit = 50 } = filters;
  ...
  async update(id: string, userId: string, data: Partial<ClientCreateDTO>): Promise<Client | null> {
    const pool = getPool();
    const fields: string[] = [];
    const values: any[] = [];
    ...
    await pool.execute(`UPDATE clients SET ${fields.join(", ")} WHERE id = ? AND user_id = ?`, values);
    return this.getById(id, userId);
  }

  async updateStatus(id: string, userId: string, status: string): Promise<Client | null> {
    const pool = getPool();
    await pool.execute("UPDATE clients SET status = ? WHERE id = ? AND user_id = ?", [status, id, userId]);
    return this.getById(id, userId);
  }
  ...
  async delete(id: string, userId: string): Promise<boolean> {
    ...
    const [result] = await pool.execute<ResultSetHeader>(
      "UPDATE clients SET is_active = FALSE WHERE id = ? AND user_id = ?", [id, userId]
    );
    return result.affectedRows > 0;
  }

  async importFromLeads(userId: string, leads: any[], source: string = "google_places"): Promise<number> {
    let imported = 0;
