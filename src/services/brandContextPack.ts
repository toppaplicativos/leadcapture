...
export function formatPackForPrompt(pack: BrandContextPack, inboundText: string): string {
  const lines: string[] = [
    ...
    `3) Depois do preço/fato, faça NO MÁXIMO 1 pergunta de próximo passo. Não abra menu genérico se a intenção já está clara.`,
    ...
    `6) MEMÓRIA: se o histórico ou a memória já trazem uso (casa/restaurante/revenda), intenção de compra, quantidade ou produto — NÃO repergunte. Avance o fluxo.`,
    `7) Se o cliente disser "quero comprar" e o uso já foi dito, ofereça produto+preço e peça só o que falta (ex.: quantidade). Nunca reinicie a segmentação.`,
