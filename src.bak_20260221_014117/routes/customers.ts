...
router.get("/stats", authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    const userId = req.user?.userId as string | undefined;
    if (!userId) return res.status(401).json({ error: "Unauthorized" });
    const stats = await customersService.getStats(userId);
...
router.get("/:id", authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    const userId = req.user?.userId as string | undefined;
    if (!userId) return res.status(401).json({ error: "Unauthorized" });
    const customer = await customersService.getById(req.params.id as string, userId);
...
router.post("/", authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const customer = await customersService.create(req.body, userId);
...
router.put("/:id", authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    const userId = req.user?.userId as string | undefined;
    if (!userId) return res.status(401).json({ error: "Unauthorized" });
    const customer = await customersService.updateCustomer(req.params.id as string, req.body, userId);
    if (!customer) return res.status(404).json({ error: "Customer not found" });
    res.json({ success: true, customer });
  } catch (error: any) {
    res.status(500).json({ error: error.message });
...
router.put("/:id/status", authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const customerBefore = await customersService.getById(req.params.id as string, userId);
    if (!customerBefore) return res.status(404).json({ error: "Customer not found" });

    const success = await customersService.updateStatus(req.params.id as string, status, userId);
...
router.delete("/:id", authMiddleware, requireRole(["admin", "manager"]), async (req: AuthRequest, res: Response) => {
  try {
    const userId = req.user?.userId as string | undefined;
    if (!userId) return res.status(401).json({ error: "Unauthorized" });
    const success = await customersService.delete(req.params.id as string, userId);
    if (!success) return res.status(404).json({ error: "Customer not found" });
