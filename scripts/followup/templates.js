 ...
 * ============================================================
 *
 * Cada template aceita um `profile` e retorna { aiPrompt, fallback }.
 * Os templates seguem a estrutura Reev Outbound (8 contatos):
 *
 *   FU0 Abertura    D+0   Grande Ideia + Problema 1
 *   FU1 Check-in    D+2   Contexto + Problema 2
 *   FU2 Conscienci. D+5   Implicacao 1 + Futuro Positivo
 *   FU3 Prova       D+8   Implicacao 2 + Prova Social
 *   FU4 Educacao    D+12  Grande Ideia + Educacao
 *   FU5 Caso Real   D+16  Storytelling + Futuro Positivo
 *   FU6 Valor Puro  D+20  Problema + Conteudo
 *   FU7 Break-up    D+25  Grande Ideia + Escassez
 *
 * Variaveis esperadas no `profile` (ver profiles/_TEMPLATE.json)
 */
...
/**
 * Cabecalho comum a todas as mensagens.
 * Define identidade do agente, empresa, produto, tom e regras globais.
 */
const header = (p) => {
...
const fu0 = (p) => ({
  ...
  slug: "fu0_abertura",
  delayDays: 0,
  sendAfterTag: null, // NAO tem pre-requisito, e o ponto de entrada
  addTag: "fu0_enviada",
...
- NAO fale de preco na primeira mensagem`,
  fallback: p.templates?.fu0 || `Oi! 😊 Sou ${p.agent.name} da *${p.company.name}*. ${p.product.shortPitch || p.company.description}. Posso te contar mais sobre como funciona?`,
});
...
NAO repita a apresentacao. Assuma que ele ja sabe quem e a ${p.company.name}.`,
  fallback: p.templates?.fu1 || `Oi! 👋 Passando aqui de novo, sou ${p.agent.name} da *${p.company.name}*. ${p.product.painLine || 'Sei que na rotina isso faz diferenca.'} Faz sentido ai no seu dia a dia?`,
});
...
Tom: reflexao, nao medo. Zero drama.`,
  fallback: p.templates?.fu2 || `[nome], ja parou pra pensar: ${p.product.costFigure || 'essa rotina custa caro no fim do mes'} 📈 Com nosso ${p.product.name}, voce ganha esse tempo de volta. Faz diferenca ai?`,
});

// ─────────────────────────────────────────────────────────────
// FU3 — Prova Social (D+8)
// ─────────────────────────────────────────────────────────────
const fu3 = (p) => ({
...
${list(p.socialProof.metrics)}
  * Hoje atendemos ${p.socialProof.clientCount} ${p.socialProof.clientType || "estabelecimentos"}
...
Tom: autoridade serena, nao pressao.`,
  fallback: p.templates?.fu3 || `[nome], ja temos ${p.socialProof.clientCount} usando nosso ${p.product.name} em ${p.target.region}. ${p.socialProof.headlineMetric || 'Economia real comprovada.'} Posso te mandar nosso catalogo sem compromisso?`,
});
...
Zero pressao comercial. Tom de compartilhamento profissional.`,
  fallback: p.templates?.fu4 || `Oi [nome]! Dica rapida: ${p.techEducation?.[0] || 'existem detalhes tecnicos que fazem toda diferenca no seu ramo'} 👀 Qualquer duvida, me chama, tenho prazer em ajudar!`,
});
...
Tom de quem conta uma historia — nao de quem vende.`,
  fallback: p.templates?.fu5 || `[nome], conta rapido: ${p.socialProof.stories?.[0] || p.socialProof.miniCases?.[0] || 'tive um cliente com cenario parecido e a transformacao foi grande'} 🚀 Imagino que faria diferenca ai tambem. Topa um orcamento rapido?`,
});
...
Tom: generosidade verdadeira, nao pega-boba. Ultimo investimento de boa vontade antes do break-up.`,
  fallback: p.templates?.fu6 || `Oi [nome]! Montei um ${p.freeContentOffers?.[0] || 'material pratico'} 📄 Mesmo que nunca vire cliente meu, esse material pode te ajudar. Posso te enviar?`,
});
...
Tom: humano, sem drama, sem carencia. Dignidade total.`,
  fallback: p.templates?.fu7 || `[nome], sera meu ultimo contato — nao quero ser chata 🙏 Se um dia precisar, salva meu numero aqui.

Antes de ir, me responde so uma coisa: voce nao se interessou porque ${(p.exitSurveyOptions || []).map((o, i) => `(${String.fromCharCode(97 + i)}) ${o}`).join(", ")}? Sua resposta me ajuda muito!`,
});

// ─────────────────────────────────────────────────────────────
// Helper para segmentos
// ─────────────────────────────────────────────────────────────
function segmentMention(p) {
...
// ─────────────────────────────────────────────────────────────
// Export: sequencia completa (FU0 ate FU7)
// ─────────────────────────────────────────────────────────────
...
module.exports = {
  buildSequence,
  templates: { fu0, fu1, fu2, fu3, fu4, fu5, fu6, fu7 },
  header,
};
