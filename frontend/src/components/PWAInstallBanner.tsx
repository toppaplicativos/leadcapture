...
export function PWAInstallBanner() {
  ...
  const benefits: { Icon: LucideIcon; text: string }[] = [
    ...
    { Icon: Bell, text: identity.app === 'affiliate' ? 'Alertas de comissões e vendas' : 'Notificações importantes' },
    { Icon: Monitor, text: 'Experiência de app nativo' },
  ]

  const title = `Instalar ${identity.name}`
  const subtitle =
    (identity.surface === 'partners' && APP_HINTS.partners)
    || APP_HINTS[identity.app]
            ...
            <div className="px-6 pt-5 pb-5">
              <div className="flex items-center justify-between mb-5">
                <h3 className="text-[16px] font-semibold tracking-tight text-gray-900">
                  Instalar {identity.name} no iPhone
                </h3>
                <button
              ...
              <ol className="space-y-3.5">
                {[
                  ...
                  { Icon: Check, text: `Confirme o nome “${identity.name}” e toque em Adicionar` },
                ].map((s, i) => (
