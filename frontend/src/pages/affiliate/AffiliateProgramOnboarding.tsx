...
export function AffiliateProgramOnboarding({ ctx, enrollmentId, onClose }: Props) {
  ...
  async function completeStep(step: AffiliateProgramStep, extra?: Record<string, unknown>) {
    if (step.locked) return ctx.showToast('Conclua a etapa anterior primeiro', 'err')
    setSubmitting(step.id)
    try {
      const res = await affiliateApi.completeOnboardingItem(enrollmentId, {
        item_type: 'step',
        item_id: step.id,
        payload: extra,
      })
      setData(res)
      ctx.showToast('Etapa concluída!')
      if (res.resources_unlocked) ctx.showToast('Link e cupom liberados!')
    } catch (e: unknown) {
      ctx.showToast(e instanceof Error ? e.message : 'Erro ao concluir etapa', 'err')
    } finally {
      setSubmitting(null)
    }
  }
  ...
  async function completeTraining(trainingId: string) {
    ...
    try {
      ...
      setData(res)
      ctx.showToast('Treinamento concluído!')
    } catch (e: unknown) {
      ctx.showToast(e instanceof Error ? e.message : 'Erro', 'err')
    } finally {
      setSubmitting(null)
    }
  }
        ...
        <div className="affiliate-card p-4 mb-3 border border-emerald-200 bg-emerald-50/50">
          ...
          <div className="mt-3 space-y-2">
            <div className="flex items-center justify-between gap-2">
              ...
              <button type="button" className="text-xs font-bold" style={{ color: ctx.primary }} onClick={() => copyText(data.enrollment_code, 'Link copiado!')}>
            ...
            <div className="flex items-center justify-between gap-2">
              ...
              <button type="button" className="text-xs font-bold" style={{ color: ctx.primary }} onClick={() => copyText(data.coupon_code, 'Cupom copiado!')}>
